        ...
        if(!start_plan && clicked){
            ...
            if(goal_point_list.size() >= 3){//设置的目标点个数大于等于3时开始规划轨迹
                ...
                for(int i=1; i<=goal_point_list.size(); i++){
                    myplanner.route(i, 0) = goal_point_list[i].pose.position.x;
                    myplanner.route(i, 1) = goal_point_list[i].pose.position.y;
                    myplanner.route(i, 2) = goal_point_list[i].pose.position.z;
                }
