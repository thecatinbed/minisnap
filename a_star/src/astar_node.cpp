        ...
        if(!start_plan && clicked){
            ...
            if(rec_goal_point.z < -100){//设置的目标点个数大于等于3时开始规划轨迹
                ...
                for(int i=1; i < goal_point_list.size(); i++){
                    myplanner.route(i, 0) = goal_point_list[i-1].x;
                    myplanner.route(i, 1) = goal_point_list[i-1].y;
                    myplanner.route(i, 2) = goal_point_list[i-1].z;
                }
                std::cout << myplanner.route << endl;
